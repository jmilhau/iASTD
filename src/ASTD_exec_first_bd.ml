...
let get_db key = State2DB.db2state key

let register_db key opt_const state astd = begin ignore(State2DB.register2db astd state key opt_const) end

let remove_db key opt_const astd= begin State2DB.deleteTuplesDB key end

let rec filter_add_remove to_save= match to_save with
|((key,opt_const),astd,state,true)::t-> (filter_add_remove t)@[((key,opt_const),astd,state,true)]
|((key,opt_const),astd,state,false)::t-> ((key,opt_const),astd,state,false)::(filter_add_remove t)
|[]->[]

let rec save_synch_data to_save = match to_save with
        |((key,opt_const),astd,state,add)::t-> 
			if add
			then begin
                                register_db key opt_const state astd; 
                                save_synch_data t
                                end
			else begin debug "remove all";
                                remove_db key opt_const astd;
                                save_synch_data t
                                end
        |[]->begin end

let get_synch_bd not_init_dom init_state key opt_const= let value = begin match opt_const with |Some c -> c | None -> failwith "No value in get_synch" end
							in if (ASTD_constant.is_included value not_init_dom)
                                                        	then State2DB.db2state key
                                                         	else init_state
...
begin debug ("is_final? @ "^current_key);
match state with
	...
	|ASTD_state.Kleene_s (started,sub_state) -> begin 
	...
	|ASTD_state.QSynchronisation_s  (not_final_domain,unknown_domain,not_init_domain,init_state) ->
		...
		in begin while !not_final=ASTD_constant.empty_dom && (!unknown)<>(ASTD_constant.empty_dom)
			do
				let (head_val,tail_val)=ASTD_constant.head_tail !unknown
				in begin debug "loop is final qsynchro";
					let bind_env=ASTD_environment.bind var (ASTD_term.Const head_val)
					in let env2=(ASTD_environment.add_binding bind_env env)
					in begin if (ASTD_constant.is_included head_val not_init_domain)
						then begin let sub_state =get_db  (current_key^":"^(ASTD_constant.string_of head_val)^"/"^(ASTD_astd.get_name sub_astd))
							...
							in if sub_final
								then begin unknown:=tail_val;
									register_db current_key (Some head_val) new_state astd
									end
								else begin unknown:=tail_val;
							...
							in if sub_final 
								then begin unknown:=tail_val end
								else begin unknown:=tail_val;(not_final := ASTD_constant.insert (ASTD_constant.value_of head_val) !not_final)end
							end
					end
				end
			done;
...
let rec modify_h hist name new_state= begin
    match hist with
    |(a,b)::q -> begin debug (name ^" vs "^ a);
                   if a=name then (name,new_state)::q
...
let rec apply_all_modifs name var astd env not_init current_key modifs study_state = 
	let astd2=ASTD_astd.get_qastd astd
	in if modifs <> []
	then let (const,(delta,key,sub_astd,new_state,to_save,kappa,is_modified))= List.hd modifs
		...
		in let (new_state2,isfinal)=(is_final astd2 new_state env2 [] key2)
		in if (ASTD_constant.is_included const not_init)
			then begin 
			if isfinal
			then begin
				...
				in apply_all_modifs name var astd env not_init current_key  (List.tl modifs) (new_study_state,key2,sub_astd2,new_study_state,((key,None),sub_astd,delta,true)::(to_save@save),kappa@kappa2,true)
				end
			else begin
			...
			then begin
				let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init);
				in apply_all_modifs name var astd env not_init current_key (List.tl modifs) (new_study_state,key2,sub_astd2,new_study_state,((current_key,Some const),astd,new_state2,true)::(to_save@save),kappa@kappa2,true)
				end
			else begin
				let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.insert value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init);
				in apply_all_modifs name var astd env  not_init current_key (List.tl modifs) (new_study_state,key2,sub_astd2,new_study_state,((current_key,Some const),astd,new_state2,true)::(to_save@save),kappa@kappa2,true)
...
let rec find_arrow arrow_list event current sub_state sub_astd env  current_key = debug "find arrow";match arrow_list with
...
then (state,"",astd,state,[],[],false) 
				...
				then let new_hist = begin if ASTD_state.is_automata new_state
					...
					in if ASTD_arrow.is_to_sub arrow
						then let dest=ASTD_arrow.get_to arrow
							...
							in begin debug ("search through "^through);let new_sub_astd = (ASTD_astd.find_subastd through sub_astd)
							    in let new_sub_state = ASTD_state.Automata_s (through,new_hist,goto_automata new_sub_astd dest new_hist)
						    	in (new_sub_state,current_key,astd,new_sub_state,[],[],true) end
						else let dest=ASTD_arrow.get_to arrow
							in begin debug ("search dest "^dest);
							    let new_sub_astd = (ASTD_astd.find_subastd dest sub_astd)
						    	in let new_sub_state = 
								(ASTD_state.Automata_s (dest,new_hist,init new_sub_astd))
							in (new_sub_state,current_key,astd,new_sub_state,[],[],true) end
				else let (delta,key,sub_astd,mod_state,to_save,kappa,is_modified) = execute new_state astd2 event env (current_key^"/"^(ASTD_astd.get_name astd2) )
  ...
  |ASTD_state.Kleene_s (started,state2) ->  
	begin debug ("kleene exec astd "^(ASTD_astd.get_name astd)^" vs position "^current_key);
	...
	in if started 
		then if is_modified
			then begin debug ("kleene modified "^name);
				...
				then begin debug ("sub astd reinit for kleene exec : "^(ASTD_astd.get_name sub_astd)) ;
					(ASTD_state.Kleene_s (false,mod_state),current_key,astd,ASTD_state.Kleene_s (false,mod_state),to_save,kappa,true)
					end
				else begin debug ("sub astd for kleene exec : "^(ASTD_astd.get_name sub_astd)) ;
				             (delta,key,sub_astd,ASTD_state.Kleene_s (started,mod_state),to_save,kappa,is_modified) 
				               end
				end
				...
				in if isfinal
					then begin debug ("restart kleene "^name); 
						...
						in if is_modified2 
							then (delta2,key2,sub_astd2,ASTD_state.Kleene_s (started,mod_state2),(((current_key^"/"^(ASTD_astd.get_name astd2)), None) ,sub_astd, delta, false)::to_save2,kappa2,is_modified2)
							else (state,"",astd,state,[],[],false) 
						end
	...
	in if val_used=ASTD_state.ChoiceNotMade
		then begin debug "trying some values";
			let (direct_val,indeterminist)=(get_values event astd2 (ASTD_term.Var var))
			 in try_qchoice dep (ASTD_event.get_label event) state2 astd var event env list_val direct_val indeterminist current_key
			end
					
		else begin 
			debug ("qchoice "^var^" choice using "^(ASTD_term.string_of(ASTD_state.get_val val_used))^" for the qastd "^name^" leading to "^(ASTD_astd.get_name astd2));
  ...
  |ASTD_state.QSynchronisation_s (not_fin_dom,unknown_dom,not_init_dom,init_state) -> 
	begin debug ("qsynch exec astd "^(ASTD_astd.get_name astd)^" vs position "^current_key);
			...
			in if not(value2=ASTD_state.ChoiceNotMade)
				...
				if (List.mem (ASTD_event.get_label event) trans_list) || (not(ASTD_constant.is_included (ASTD_term.extract_constant_from_term (ASTD_state.get_val value2)) val_list))
					then (state,"",astd,state,[],[],false)
						...
						in let get_state=get_synch_bd not_init_dom init_state  (current_key^":"^(ASTD_constant.string_of (ASTD_term.extract_constant_from_term (ASTD_state.get_val value2)))^"/"^(ASTD_astd.get_name astd2)) (Some value)   
								...
								then begin debug "kappa indirect !!!!!!!!!!";
									...
									then let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value' not_fin_dom),(ASTD_constant.remove value' unknown_dom),(ASTD_constant.insert value' not_init_dom),init_state)
										in (new_study_state,
											...
											astd,
											new_study_state,
											((key,None),sub_astd,delta,true)::to_save,
											kappa,
											is_modified)
									...
									then let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value' not_fin_dom),(ASTD_constant.remove value' unknown_dom),(ASTD_constant.insert value' not_init_dom),init_state)
										in (new_study_state,
											...
											astd,
											new_study_state,
											((key,Some value),astd,new_state,true)::to_save,
											kappa,
											is_modified)
					...
					then let (list_modif,modif_possible)=modif_all_qsynch name astd event init_state not_init_dom val_list env [] var  current_key
						in if modif_possible 
							then begin 
								apply_all_modifs name var astd env  not_init_dom current_key list_modif (state,"",astd,state,[],[],false)
								end
							else (state,"",astd,state,[],[],false)
					else let (direct_val,indeterminist)= (get_values event astd2 (ASTD_term.Var var))
						in try_qsynch name state astd var event env val_list direct_val val_list trans_list indeterminist current_key
					end		
	end 

	with _ -> begin debug "catched error 1";(state,"",astd,state,[],[],false) end
	end 
  ...
  |_ -> begin debug "error catched 2"; (state,"",astd,state,[],[],false) end
	...
	then if indeterminist
		then if not (list_val = (ASTD_constant.empty_dom))
			then begin debug "then trying global values";
				...
				in let env2=(ASTD_environment.add_binding bind_env env)
				in let (delta,key,sub_astd,mod_state,to_save,kappa,is_modified) = execute state astd2 event env2 (current_key^":"^(ASTD_constant.string_of head_val)^"/"^(ASTD_astd.get_name astd2))
				in if is_modified
					then begin 
		...
		else begin debug "indeter" ;(state,"",astd,state,[],[],false) end
		...
		then try_qchoice dep label state astd var event env list_val (List.tl kappa_dir_val) indeterminist current_key
			...
			in let env2=(ASTD_environment.add_binding bind_env env)
			in let (delta,key,sub_astd,mod_state,to_save,kappa,is_modified) = execute state astd2 event env2  (current_key^":"^(ASTD_constant.string_of (ASTD_term.extract_constant_from_term head_val))^"/"^(ASTD_astd.get_name astd2))
			in if is_modified
...
and try_qsynch name state astd var event env list_val kappa_dir_val dom trans_list indeterminist current_key=
let astd2=ASTD_astd.get_qastd astd
in if kappa_dir_val=[]
	then if indeterminist
		then if list_val <> (ASTD_constant.empty_dom)
			...
			in let bind_env = ASTD_environment.bind var (ASTD_term.Const head_val)
			in let env2=(ASTD_environment.add_binding bind_env env)
			in let sub_state=get_synch_bd not_init_dom init_state (current_key^":"^(ASTD_constant.string_of head_val)^"/"^(ASTD_astd.get_name astd2)) (Some head_val)   
			in let (delta,key,sub_astd,mod_state,to_save,kappa,is_modified) = execute sub_state astd2 event env2   (current_key^":"^(ASTD_constant.string_of head_val)^"/"^(ASTD_astd.get_name astd2))
			in if is_modified
				then let (new_state,isfinal)=(is_final astd2 mod_state env2 [] (current_key^":"^(ASTD_constant.string_of head_val)^"/"^(ASTD_astd.get_name astd2)))
					in if (ASTD_constant.is_included head_val not_init_dom)
						then begin debug "modified1 in qsynch";
						if isfinal
						then let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
							in (new_study_state,current_key,astd,new_study_state,((key,None),sub_astd,delta, true)::to_save,kappa,is_modified)
						else let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.insert value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
							in (new_study_state,current_key,astd,new_study_state,((key, None),sub_astd,delta, true)::to_save,kappa,is_modified)
						end
						else begin debug "modified2 in qsynch";
						if isfinal
						then let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
				...
				else try_qsynch name state astd var event env tail [] dom trans_list indeterminist current_key
			end
			else begin debug "error1"; (state,"",astd,state,[],[],false) end
		else begin debug "indeter" ;(state,"",astd,state,[],[],false) end
		...
		then begin debug "error3"; try_qsynch name state astd var event env list_val (List.tl kappa_dir_val) dom trans_list indeterminist current_key end
			...
			in if is_modified
				then let (new_state,isfinal)=(is_final astd2 mod_state env2 [] (current_key^":"^(ASTD_constant.string_of head_val)^"/"^(ASTD_astd.get_name astd2)))
					in if (ASTD_constant.is_included head_val not_init_dom)
						then begin debug "kappa direct1 !!!!!!!!!!";
						if isfinal
						then let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
							in (new_study_state,current_key,astd,new_study_state,((key, None) ,sub_astd, delta, true)::to_save,kappa,is_modified)
						else let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.insert value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
						...
						else begin debug "kappa direct2 !!!!!!!!!!";
						if isfinal
						then let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.remove value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
							in (new_study_state,current_key,astd,new_study_state,((current_key, Some head_val) ,astd, new_state, true)::to_save,kappa,is_modified)
						else let new_study_state=ASTD_state.QSynchronisation_s((ASTD_constant.insert value not_fin_dom),(ASTD_constant.remove value unknown_dom),(ASTD_constant.insert value not_init_dom),init_state)
				...
				else begin debug "error4" ; try_qsynch name state astd var event env list_val (List.tl kappa_dir_val) dom trans_list indeterminist current_key end
		end


and modif_all_qsynch name astd event init_state not_init_dom value_list env modifs var current_key=
	let astd2=ASTD_astd.get_qastd astd
	in if value_list<>ASTD_constant.empty_dom
		then let (val_used, tail)= ASTD_constant.head_tail value_list
			...
			in if ASTD_constant.is_included val_used not_init_dom
				then begin 
					let state=get_db (current_key^":"^(ASTD_constant.string_of val_used)^"/"^(ASTD_astd.get_name astd2))
					in let (delta,key,sub_astd,mod_state,to_save,kappa,is_modified) = execute state astd2 event env2 (current_key^":"^(ASTD_constant.string_of val_used)^"/"^(ASTD_astd.get_name astd2))
						in let modif=(val_used,(delta,key,sub_astd,mod_state,to_save,kappa,is_modified))
						in if is_modified 
							then modif_all_qsynch name astd event init_state not_init_dom tail env (modif::modifs) var current_key
							else ([],false)
					end
				else begin 
					let (delta,key,sub_astd,mod_state,to_save,kappa,is_modified) = execute init_state astd2 event env2 (current_key^":"^(ASTD_constant.string_of val_used)^"/"^(ASTD_astd.get_name astd2))
						in let modif=(val_used,(delta,key,sub_astd,mod_state,to_save,kappa,is_modified))
						in if is_modified 
							then modif_all_qsynch name astd event init_state not_init_dom tail env (modif::modifs) var current_key
							else ([],false)
...
let rec print state astd s current_key= match state with
        |ASTD_state.Automata_s (a,b,c) ->print_newline();
                              ...
                              (print_h b astd (s^"//") (ASTD_astd.get_name astd) current_key) ;*)
        ...
        |ASTD_state.QSynchronisation_s (not_fin,unknown,not_init_dom,init_state) -> print_newline();print_endline(s^"QSynchronisation_s ,Not_Initial values:  "^(ASTD_constant.print_dom not_init_dom)^" for astd "^(ASTD_astd.get_name astd)^" @ "^current_key);
...
and print_h hist astd s name current_key= match hist with
  |(n1,h)::t ->print_endline(s^n1^" @ "^current_key^" AKA "^name );
               print h (ASTD_astd.find_subastd n1 (ASTD_astd.get_sub astd)) (s) (current_key^"/"^n1);
               print_h t astd s name current_key
    ...
    if ASTD_constant.is_empty_dom not_init 
        then print_newline ()
            ...
            in begin 
                ...
                print (get_db (current_key^":"^(ASTD_constant.string_of value)^"/"^(ASTD_astd.get_name sub_astd)))
                    (sub_astd)
                    s
...
let rec execute_event_list affichage wait state astd event_list time= match event_list with
	|event::tail->  
			begin begin if wait then begin print_endline "Ready !" ; ignore(input_line stdin) end else begin end end;
			if affichage<>1 
			then 
			begin
			print_endline "================================================================";
			...
			in let start_time=(Unix.gettimeofday())
			in begin 
				if is_modified 
				then if affichage=2
					then 
						begin debug ("executed at "^key);ignore(compteur());
						save_synch_data (filter_add_remove to_save);
						begin register_db key None delta delta_astd ;
						...
						begin debug ("executed at "^key);ignore(compteur());
						save_synch_data (filter_add_remove to_save);
						register_db key None delta delta_astd ;
						ASTD_optimisation.apply_each_mod kappa ;
				...
				let end_register=Unix.gettimeofday()
				in begin 
					print_float(end_register-.start_time);
					print_endline " seconds of Registration";
					execute_event_list affichage wait (get_db "Main") astd tail (time+.(end_register-.start_time))
				end
			end
			else
			begin 
			...
			in begin 
				if is_modified 
				then begin debug ("executed at "^key);
				        ignore (compteur());
						save_synch_data (filter_add_remove to_save);
						register_db key None delta delta_astd ;
						ASTD_optimisation.apply_each_mod kappa
				...
				let end_register=Unix.gettimeofday()
				in execute_event_list affichage wait (get_db "Main") astd tail (time+.(end_register-.start_time))
			end end
	|[]->begin print_endline "================================================================";(state,time,(compteur())-1) end
